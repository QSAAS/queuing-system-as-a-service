...
import AdministratedQueueServerMother from "@tests/Command/Domain/Entity/Mother/AdministratedQueueServerMother";

describe("test Administrated Queueing Server", () => {
  describe("Events", () => {
    const administratedQueueServer: AdministratedQueueServer = AdministratedQueueServerMother.withPassingAuth().build();
  ...
  describe("Exceptions", () => {
    it("throws exception when QueueAdministratedAuthorizationService fails to updated served Queue Nodes", () => {
      const administratedQueueServer: AdministratedQueueServer =
        AdministratedQueueServerMother.withFailingAuth().build();
      expect(() => {
        administratedQueueServer.setServedQueueNodes(administratedQueueServer.getServes());

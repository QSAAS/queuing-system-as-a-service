...
export default class Permission extends ValueObject {
  ...
  public static newCreate(resourceType: ResourceType): Permission {
    return new Permission(null, resourceType, AuthorizedAction.CREATE);
  }

  public static newUpdate(resourceType: ResourceType, entityId: EntityId): Permission {
    return new Permission(entityId, resourceType, AuthorizedAction.UPDATE);
  }

  public static newDelete(resourceType: ResourceType, entityId: EntityId): Permission {
    return new Permission(entityId, resourceType, AuthorizedAction.DELETE);
  }

  public static newManage(resourceType: ResourceType, entityId: EntityId): Permission {
    return new Permission(entityId, resourceType, AuthorizedAction.MANAGE);
  }
  ...
  public equals(other: Permission): boolean {
    ...
    // OR both are null
    const equalIds =
      (this.resourceId && other.resourceId && this.resourceId.equals(other.resourceId)) ||
      this.resourceId === other.resourceId;

    return equalIds && this.resourceType === other.resourceType && this.action === other.action;
  }
}
